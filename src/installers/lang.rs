...
pub fn install_python() -> Result<InstallStatus> {
    if cmd::command_exists("python3") && cmd::command_exists("pip3") {
        ...
        // Ensure pipx and uv are installed even if Python is already there
        if !cmd::command_exists("pipx") {
            let _ = apt::install(&["pipx"]);
            let _ = cmd::run_cmd("pipx", &["ensurepath"]);
        }

        if !cmd::command_exists("uv") {
            println!("⏳ Installing uv...");
            let _ = cmd::run_cmd("bash", &["-c", "curl -LsSf https://astral.sh/uv/install.sh | sh"]);
        }
    ...
    apt::install(&["python3", "python3-pip", "python3-dev", "python3-venv", "pipx"])?;
