...
| **Modern Programming Languages** | `Node.js` (LTS) + `pnpm`<br>`Python3` + `pip` + `venv`<br>`Rust` (`cargo`, `rustup`)<br>`Go` (golang)<br>`Bun` | Abandons nvm/pyenv, adopting **global installation** or **symlink mapping**, completely solving the issue of AI not finding language environments |
